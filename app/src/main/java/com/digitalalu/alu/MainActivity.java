...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView sWin, sPcs, sPipe, sGlass;
    ...
    private void buildViews() {
        ...
        sWin = sheetView.findViewById(R.id.sWin);
        sPcs = sheetView.findViewById(R.id.sPcs);
        sPipe = sheetView.findViewById(R.id.sPipe);
        sGlass = sheetView.findViewById(R.id.sGlass);
    ...
    private void addNextRow() {
        ...
        if (!items.isEmpty()) {
            ...
            sutter = last.sutter;
            sys = last.system;          // same system as previous row
        }
    ...
    private void showRowDialog(WindowItem it) {
        ...
        /* live preview ke liye temp copy */
        ...
        final Runnable refresh = new Runnable() {
            @Override public void run() {
                ...
                /* manual value jo list me nahi hai */
    ...
    private void recalc() {
        ...
        if (sWin != null) {
            sWin.setText(String.valueOf(grand.windows));
            sPcs.setText(String.valueOf(grand.pcs));
            sPipe.setText(grand.zedPipes + " / " + grand.domalPipes);
            sGlass.setText(String.format(Locale.US, "%.1f", grand.glassSqft));
        }
    ...
    /** one system's pipe requirement table */
    ...
    private void showFormula() {
        String s2 = "FRAME  =  2xH + 2xW\n"
                + "   48 x 48  \u2192  192\"\n\n"
                + "\u2500\u2500 ZED \u2500\u2500\n"
                + "SUTTER H = H - " + st.z_dh + "\n"
                + "SUTTER W = ( W - (q+1) x " + st.z_dw + " ) / q\n"
                + "MULIYA H = H - " + st.z_dm + "   (qty = sutter - 1)\n"
                + "RP length = W - " + st.z_rpDed + "\n"
                + "GLASS = sutter - " + st.z_dg + "\n\n"
                + "\u2500\u2500 DOMAL \u2500\u2500\n"
                + "SUTTER H = H - " + st.d_dh + "\n"
                + "SUTTER W = W / q"
                + (st.d_dw != 0 ? "  (- " + st.d_dw + " each)" : "") + "\n"
                + "RT H = H - " + st.d_dm + "   (qty = sutter - 1)\n"
                + "RP length = W - " + st.d_rpDed + "\n"
                + "GLASS = sutter - " + st.d_dg + "\n\n"
                + "\u2500\u2500 RP GRILL \u2500\u2500\n"
                + "space = ( mid pipe - " + st.rpGap + " ) / qty\n"
                + "valid range " + st.rpMin + "\" to " + st.rpMax + "\"\n"
                + "auto qty shown in RP column \u2014 editable";
        new AlertDialog.Builder(this).setTitle("Formula").setMessage(s2)
                .setPositiveButton("OK", null).show();
    }

    private void showAbout() {
        new AlertDialog.Builder(this).setTitle("ALU Window Optimizer")
                .setMessage("Native Android app\nVersion 1.2\n\n"
                        + "DIGITAL ALU. SECTION\nWANKANER 363621\nMO. 99799 71170")
                .setPositiveButton("OK", null).show();
    }
    ...
    /** card = outer (add karne ke liye) + body (content dalne ke liye) */
